    ...
    Process {
        ...
        command: {
            ...
            if (root._forceFetchMode) {
                ...
                cmd = Caching.kairoDir + "/scripts/weather.sh --getdata --location '" + locEscaped + "' --unit '" + curUnit + "' && cat \"" + cacheFile + "\"";
            } else {
                cmd = Caching.kairoDir + "/scripts/weather.sh --json --location '" + locEscaped + "' --unit '" + curUnit + "'";
            }
            return ["bash", "-c", cmd];
        }
